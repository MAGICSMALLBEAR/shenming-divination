## 2026-07-26 上香互動修復與神明日曆完整擴充

### 今日目標
修復點香後無法插入香爐及插香動畫不流暢的問題，並把零散的神明紀念日功能擴充成可查詢、可追蹤、可提醒及可匯出的完整神明日曆。

### 今日完成

#### 1. 上香與插香互動修復
- 修正香點燃後無法插入香爐的流程阻塞。
- 重新整理點香、移動、插入、完成之間的狀態切換，避免重複觸發及狀態不同步。
- 改善插香位移、旋轉、落點與回彈節奏，使香枝進入香爐時更連續自然。
- 補上低動態模式及互動回歸測試。

#### 2. 每日神明與神明日曆核心
- 新增每日推薦神明及今日神諭卡片。
- 建立以 Intl 中文農曆為基礎的跨年份日期換算。
- 統一今日頁、節慶、廟宇及通知使用的農曆與神明紀念日資料來源。
- 首批收錄 17 筆臺灣民間信仰常見神明紀念日，保留地區與宮廟傳承差異提示。
- 補上資料來源、校勘狀態、檢視日期、常見供品與參拜說明。

#### 3. 神明紀念日詳細頁
- 可查看下一次國曆日期、農曆日期、神明資料及傳承說明。
- 顯示常見供品、祈願方向、參拜提醒與資料來源。
- 保留向此神明求籤入口。
- 新增單筆 ICS 匯出及加入手機系統行事曆操作。

#### 4. 格狀神明日曆
- 新增固定六週、星期日起始的月曆格狀介面。
- 支援上一月、下一月、上一年、下一年及回到本月。
- 可點選日期查看當日紀念日，並標示今天與有事件的日期。
- 支援依神明篩選及關鍵字搜尋。
- 保留整月紀念日清單、詳細資料與求籤入口。

#### 5. 行事曆匯出與原生整合
- 新增整年度 ICS 匯出；Web 版直接下載，手機版使用系統分享面板。
- ICS 使用全天事件、非包含式結束日期、唯一 UID、正確跳脫及 CRLF 格式。
- iOS／Android 可開啟系統新增事件畫面，由使用者最後確認。
- 加入 expo-calendar、expo-file-system、expo-sharing 及原生權限說明。
- 補上 Web、安裝版 App、Expo Go 及 Android 儲存狀態的介面提示。

#### 6. 我的神明日曆
- 使用者可在紀念日清單直接關注或取消關注神明。
- 新增「只看已關注」模式。
- 顯示未來 120 天內最近三筆已關注神明紀念日。
- 關注名單與設定頁的「只提醒關注神明」共用資料。
- 關注名單改變時，若已開啟紀念日通知，會同步更新通知排程。
- 支援全部或指定神明、當天或提前 1／3／7 天，以及三種提醒時段。

### 主要新增檔案
- src/services/lunarDeityCalendar.ts
- src/services/dailyDeityOracle.ts
- src/services/deityReminderPreferences.ts
- src/services/deityCalendarGrid.ts
- src/services/deityCalendarIcs.ts
- src/services/deityCalendarExport.ts
- src/services/deityCalendarFollowing.ts
- src/components/home/DailyDeityCard.tsx
- src/components/home/DeityMonthCalendar.tsx
- src/components/home/DeityCalendarExplorer.tsx
- src/app/deity-calendar/[id].tsx

### 今日驗證
- TypeScript：✅ 通過，0 個型別錯誤。
- Jest：✅ 26 組測試、154 項測試全部通過。
- Expo Lint：✅ 0 個錯誤；仍有 8 個本輪之前就存在的警告。
- Web/PWA：✅ 靜態匯出成功，共 37 條路由。
- 新增測試涵蓋農曆跨年換算、月曆格狀範圍、搜尋篩選、ICS 格式、個人關注及介面操作。

### 目前狀態與限制
- 本輪程式與測試均已完成，但工作目錄仍有尚未 commit／push 的變更。
- 手機直接加入行事曆需要重新建立 development 或 production build；Expo Go 可能不支援。
- iOS／Android 權限視窗、系統事件編輯畫面及實際儲存結果仍需實機人工驗收。
- 目前先收錄 17 筆常見紀念日，尚未涵蓋成道、得道、出家、飛昇及地方祭典。
- 安裝套件時 npm 回報 16 個相依套件弱點（14 moderate、2 high）；尚未執行可能大幅改版的自動修復。

### 未來代辦（依建議順序）

#### P0：實機驗收與本輪收尾
1. 建立 Android development build，測試行事曆權限允許、拒絕、再次要求、事件儲存與取消。
2. 建立 iOS development build，確認 write-only 權限、預設行事曆及全天事件日期。
3. 在 Google Calendar、Apple Calendar、Outlook 匯入單筆及整年 ICS，確認中文、換行、日期與重複事件行為。
4. 測試通知與關注名單同步，包括全部神明、指定神明、空名單及跨年份排程。
5. 在低階 Android、iPhone、平板與桌面 Web 檢查月曆格寬度、文字截斷、篩選及捲動。
6. 確認本輪變更範圍後 commit、push，並檢查部署結果。

#### P1：神明日曆內容擴充
1. 將 17 筆紀念日擴充為較完整的聖誕、成道日、得道日、出家日與飛昇日資料。
2. 每筆資料加入事件類型，不再全部以聖誕或單一紀念日呈現。
3. 建立全臺常見、特定地區、特定宮廟及特定法脈的傳承層級。
4. 同一神明存在不同日期時，允許使用者選擇採用的傳承版本。
5. 優先採政府、宮廟官方、文史館及學術資料，記錄來源版本與最後校勘日期。
6. 補上資料缺漏、來源失效及日期爭議的自動稽核測試。

#### P2：訂閱與同步
1. 提供 WebCal 訂閱網址，讓手機行事曆在資料更新後自動同步。
2. 支援只訂閱已關注神明、全部神明或特定地區傳承。
3. 為 ICS／WebCal 加入版本號與取消事件機制，避免更新後產生重複事件。
4. 評估登入後跨裝置同步關注名單、提醒偏好及選用傳承版本。
5. 增加通知點擊後直接開啟對應神明紀念日詳細頁。

#### P3：資料維護與發布品質
1. 建立神明日曆資料匯入或管理工具，避免長期直接修改大型 TypeScript 陣列。
2. 補上日曆頁英文、日文翻譯與字串集中管理。
3. 修正現有 8 個 lint 警告。
4. 逐項審查 16 個 npm 相依套件弱點，僅在確認 Expo SDK 相容後升級。
5. 完成原生日曆與通知端對端測試及上架前隱私權說明。

### 下一個建議工作
先完成 Android／iOS development build 的原生日曆與通知實機驗收；確認行為穩定後，再進入神明紀念日資料擴充與傳承版本階段。

---

## 2026-07-25 五款神明占卜開場動畫

### 今日目標
為 App 補上符合廟宇、祈願與占卜氛圍的正式開場動畫，取代專案內未啟用的 Expo 範例啟動畫面，並兼顧 Web、原生裝置與無障礙需求。

### 今日完成

| 項目 | 狀態 | 說明 |
|------|------|------|
| 五款開場主題 | ✅ 完成 | 廟門初啟、一炷心香、神印開示、蓮華清淨、金光引路 |
| 啟動流程整合 | ✅ 完成 | 在 Root Layout 掛載開場層，每次啟動隨機顯示其中一款 |
| 品牌視覺 | ✅ 完成 | 統一使用朱紅、暗褐與金色，加入神明占卜標題及各主題祝語 |
| 互動控制 | ✅ 完成 | 動畫約 3 秒自動結束，右上角可立即略過 |
| 無障礙 | ✅ 完成 | 加入畫面與略過按鈕標籤；系統開啟減少動態效果時改用短版靜態神印 |
| 自動測試 | ✅ 完成 | 驗證五款主題完整、隨機區間映射及異常值安全處理，共 3 項測試 |

### 主要變更
- 新增 `src/components/OpeningCeremony.tsx`：集中管理五款開場、動畫時序、文案、略過及低動態模式。
- 修改 `src/app/_layout.tsx`：在所有路由上方掛載開場動畫。
- 新增 `__tests__/openingCeremony.test.ts`：固定五款主題與選擇邏輯的回歸測試。

### 今日驗證
- `npm exec tsc -- --noEmit`：✅ 通過
- `npm run lint -- --no-cache`：✅ 通過，0 警告
- `npm test -- --runInBand __tests__/openingCeremony.test.ts`：✅ 1 組、3 項測試全過
- `npm run export:web`：✅ Web/PWA 匯出成功，共 32 條靜態路由
- 匯出期間僅出現既有 Sentry 專案環境變數未設定提示，不影響本次建置。

### 目前狀態
- 程式、測試與 Web 輸出均已完成。
- 本輪 3 個檔案仍在工作目錄，尚未 commit／push。
- 尚未逐款在 Android、iPhone、平板與不同視窗尺寸進行肉眼 QA。

### 未來代辦（依優先順序）

#### P0：本次功能收尾
1. 在手機、平板與桌面 Web 各預覽五款動畫，確認文字、神印、蓮花、香煙及廟門沒有裁切。
2. 在 Android 與 iPhone 實機確認啟動銜接、略過反應、返回前景與低動態模式。
3. 量測低階裝置的啟動時間、動畫 FPS 與記憶體；必要時減少光芒及花瓣數量。
4. 完成視覺 QA 後 commit 並 push 本輪變更。

#### P1：體驗補強
1. 在設定頁加入開場動畫選擇器，可選隨機或固定其中一款，並提供即時預覽。
2. 增加播放頻率設定：每次啟動、每日首次啟動或只在首次使用播放。
3. 視需要加入非常輕的鐘聲或木魚聲，預設關閉並遵守現有音效設定。

#### P2：發布前整合
1. 將原生靜態 Splash 圖與本次朱紅金色開場視覺再對齊，減少畫面切換感。
2. 補 Android、iOS 與 Web 的開場畫面快照或端對端測試。
3. Firebase、IAP、Apple 登入、Sentry 正式專案及原生後端部署仍依下方全站代辦清單處理。
...
> 2026-07-25 更新：五款開場動畫已完成程式與自動驗證，尚待跨裝置視覺 QA。全站其餘主要缺口多數需要外部帳號、服務或實機才能完成。
...
| # | 項目 | 現況 | 說明 |
|---|------|------|------|
| 4 | **Sentry 正式專案 / Analytics** | ⚠️ SDK 已接 | Sentry SDK 與初始化程式已完成，但匯出時仍提示 organization、project 環境設定缺失；需建立正式專案並設定部署環境變數。Analytics 尚未接入 |
...
### ✅ 近期已完成（2026-07-12 ~ 2026-07-25）

| # | 項目 | 日期 |
|---|------|------|
| — | 五款神明占卜開場動畫（隨機輪替、略過、低動態模式） | 7/25 |
